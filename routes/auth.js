...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const user = await db.users.insert({ first_name, last_name, phone, password_hash: hash, role: 'employee' });
    const token = jwt.sign({ id: user.id, role: 'employee' }, SECRET, { expiresIn: '30d' });
    res.json({ token, role: 'employee', name: first_name + ' ' + last_name });
  } catch (e) {
